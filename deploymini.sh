#!/usr/bin/env bash
# Open-AutoGLM Termux 最小化部署脚本
# 版本: 5.0.0 (移除冗余依赖，保留完整功能)
set -euo pipefail
...
uninstall_full() {
  ...
  # 6. 在 Termux 中询问是否卸载系统包
  if in_termux; then
    echo -e "${CYAN}━━━ 第 6 步：Termux 系统包 ━━━${NC}"
    ...
    # 其他工具
    echo -e "${BLUE}[INFO]${NC} 其他系统包（git, android-tools）建议手动卸载"
    echo -e "${YELLOW}命令: pkg uninstall git android-tools${NC}"
    echo
  fi
...
main() {
  ...
  echo -e "${BLUE}║${NC}       ${CYAN}版本: 5.0.0 (最小化依赖版)${NC}                         ${BLUE}║${NC}"
  echo -e "${BLUE}╚══════════════════════════════════════════════════════════════╝${NC}"
  echo
