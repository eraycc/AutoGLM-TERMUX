...
init_i18n() {
  ...
  # GitHub 加速源提示
  I18N["git_mirror_prompt_cn"]="请输入 GitHub 加速源完整 URL（末尾不要带 / ），例如：https://gitclone.com（直接回车使用官方源，输入 default 使用内置加速源）"
  I18N["git_mirror_prompt_en"]="Enter the full GitHub mirror URL (no trailing /), e.g. https://gitclone.com (Enter=official, 'default'=built-in)"
  I18N["git_mirror_builtin_cn"]="将使用内置加速源："
  I18N["git_mirror_builtin_en"]="Using built-in mirror:"
  I18N["git_mirror_custom_cn"]="将使用自定义加速源："
  I18N["git_mirror_custom_en"]="Using custom mirror:"
  I18N["git_mirror_official_cn"]="将使用官方仓库地址"
  I18N["git_mirror_official_en"]="Using official repo URL"
...
clone_or_update() {
  ...
  # ---------- 多语言加速源选择 ----------
  local repo_url="https://github.com/zai-org/Open-AutoGLM"
  echo
  ...
  case "$mirror_input" in
    "")
      ...
      ;;
    default|DEFAULT)
      repo_url="https://gitclone.com/${repo_url}"
      echo -e "${BLUE}[INFO]${NC} $(i18n git_mirror_builtin) ${GREEN}$repo_url${NC}"
      ;;
    *)
      # 用户输入完整 URL，直接拼接
      repo_url="${mirror_input}/${repo_url}"
      echo -e "${BLUE}[INFO]${NC} $(i18n git_mirror_custom) ${GREEN}$repo_url${NC}"
      ;;
  esac
  # ---------- 加速源处理结束 ----------
